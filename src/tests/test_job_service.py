...
def test_job_service_generate_subtitles(tmp_path):
    ...
    with (
        patch("homelab_subs.services.job_service.MONITORING_AVAILABLE", True),
        patch("homelab_subs.services.job_service.DB_LOGGING_AVAILABLE", True),
        patch("homelab_subs.services.job_service.PipelineRunner") as MockRunner,
    ):
        ...
        result = service.generate_subtitles(
            video_path=video, output_path=output, job_id="job42"
        )
...
def test_job_service_history_helpers(tmp_path):
    ...
    with (
        patch("homelab_subs.services.job_service.DB_LOGGING_AVAILABLE", True),
        patch("homelab_subs.services.job_service.DatabaseLogger") as MockLogger,
    ):
        ...
        service = JobService(
            enable_monitoring=False, enable_db_logging=True, db_path=db_path
        )
