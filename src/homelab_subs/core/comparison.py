...
class SubtitleComparator:
    ...
    def compare_cues(
        ...
        text_metrics = self._compute_text_metrics(
        ...
        for ref_cue in reference_cues:
            ...
            for idx, hyp_cue in enumerate(hypothesis_cues):
                ...
                overlap = calculate_overlap_ratio(
                    ref_cue.start_seconds,
                    ref_cue.end_seconds,
                    hyp_cue.start_seconds,
                    hyp_cue.end_seconds,
                )
    ...
    def _compute_timing_metrics(
        ...
        for c in comparisons:
            overlap = calculate_overlap_ratio(
                c.reference.start_seconds,
                c.reference.end_seconds,
                c.hypothesis.start_seconds,
                c.hypothesis.end_seconds,
            )
    ...
    def _compute_overall_score(
        ...
        timing_score = (
            timing_metrics.overlap_ratio * 50 + timing_metrics.timing_accuracy_pct * 0.5
        )

        # Segment score: based on match ratio and segmentation similarity
        if segment_metrics.reference_count > 0:
            match_ratio = (
                segment_metrics.matched_count / segment_metrics.reference_count
            )
        else:
            match_ratio = 1.0 if segment_metrics.hypothesis_count == 0 else 0.0

        segment_score = match_ratio * 50 + segment_metrics.segmentation_similarity * 50

        # Weighted combination
        overall = text_score * 0.50 + timing_score * 0.25 + segment_score * 0.25
...
def format_comparison_report(result: ComparisonResult, detailed: bool = False) -> str:
    ...
    if detailed and result.segment_comparisons:
        ...
        for i, comp in enumerate(result.segment_comparisons[:10]):
            lines.append(f"\nSegment {i + 1}:")
            lines.append(
                f'  Reference: "{comp.reference.text[:50]}{"..." if len(comp.reference.text) > 50 else ""}"'
            )
            lines.append(
                f'  Hypothesis: "{comp.hypothesis.text[:50]}{"..." if len(comp.hypothesis.text) > 50 else ""}"'
            )
            lines.append(
                f"  Similarity: {comp.text_similarity:.2%}, WER: {comp.word_error_rate:.2%}"
            )
            lines.append(
                f"  Time offset: start={comp.start_offset_ms:+.0f}ms, end={comp.end_offset_ms:+.0f}ms"
            )
